...
class UnlockFunds(AbstractMethod):
    ...
    async def run(self, user):
        ...
        payments = await user.execute(locked_payments_method)
        save_back = []
        for paym in payments:
            if paym['pay_req'] != self.pay_req:
                save_back.append(paym)

        await user.ctx.redis.delete('locked_payments_for_' + user.userid)
        for doc in save_back:
            await user.ctx.redis.rpush('locked_payments_for_' + user.userid, json.dumps(doc))